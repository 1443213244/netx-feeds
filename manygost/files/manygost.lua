...
function addServer(lastservers, lastnodes, port, localServers)
    ...
    file = io.open('/etc/manygost/server.json', 'w')
    file:write(text)
    file:close()
end
...
function GostConf(servers)
    ...
    file = io.open('/etc/manygost/config.json', 'w')
    file:write(content)
    file:close()
end

function main()
    localServers = getServer('/etc/manygost/server.json')
    lastServers = getServer('/etc/manygost/lastserver.json')
